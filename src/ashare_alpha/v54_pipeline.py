...
from .model_v54 import CONTROL_COLUMNS_V54, rank_feature_matrix_v54


def build_model_source_panel_v54(
    *,
    ...
    return (
        panel,
        rank_feature_matrix_v54(panel),
        coverage[coverage["date"].between(start_date, end_date)].copy(),
        return_manifest,
    )


def neutralize_model_predictions_v54(
    panel: pd.DataFrame,
    ...
    *,
    ...
    if not predictions.index.equals(panel.index):
        raise ValueError("V54 predictions do not align with the source panel")
    source = panel.copy()
    ...
    for _, rows in source.groupby("date", sort=True):
        ...
        if not scored.empty:
            parts.append(scored)
    if not parts:
        raise ValueError("V54 produced no complete neutralized cross-section")
