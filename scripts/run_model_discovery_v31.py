...
from ashare_alpha.evaluate import _neutralize_cross_section, evaluate_research, exposure_diagnostics
...
from ashare_alpha.model_v31 import RIDGE_PARAMETERS_V31, fit_model_v31, interaction_matrix_v31
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train and validate the frozen V31 interaction factor.")
    ...
    if protocol["candidate_registry"] != ["rpsi_v31"]:
        raise ValueError("V31 protocol registry mismatch")
    if protocol["training"]["fixed_hyperparameters"] != RIDGE_PARAMETERS_V31:
        raise ValueError("V31 ridge parameters do not match the frozen protocol")
    ...
    if len(prior) != expected_prior or prior["candidate"].nunique() != expected_prior:
        raise ValueError(f"V31 requires exactly {expected_prior} unique prior tests")
    ...
    matrix, model_columns, polynomial = interaction_matrix_v31(panel)
    target = residualize_training_target_v30(panel)
    model = fit_model_v31(matrix, target, panel["date"])
    panel["rpsi_v31"] = model.predict(matrix)

    neutralized = []
    for _, rows in panel.groupby("date", sort=True):
        result = _neutralize_cross_section(
            rows, "rpsi_v31", KNOWN_FEATURES_V30,
            minimum_rows=int(protocol["evaluation_neutralization"]["minimum_rows"]),
        )
        if not result.empty:
            neutralized.append(result)
    if not neutralized:
        raise ValueError("V31 produced no neutralized month")
    scored = pd.concat(neutralized, ignore_index=True)
    definition = {
        "rpsi_v31": Candidate(
            "regularized_pairwise_structural_interactions",
            "Fixed ridge-regularized pairwise rank interactions predict returns beyond all known registered main effects.",
        )
    }
    ...
    with model_path.open("wb") as handle:
        pickle.dump(
            {"model": model, "columns": model_columns, "polynomial": polynomial},
            handle,
            protocol=5,
        )
    ...
    selection = {
        ...
        "interaction_column_count": len(model_columns),
        "return_manifest": return_manifest,
        "passed_candidates": summary.loc[summary["passes_research_gate"], "candidate"].tolist(),
        "selected_candidate": "rpsi_v31" if bool(summary.iloc[0]["passes_research_gate"]) else None,
