...
from ashare_alpha.model_v52 import CANDIDATES_V52, predict_frozen_model_v52
from ashare_alpha.v52_pipeline import (
    build_model_source_panel_v52,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run one-shot asset-and-time-disjoint V52 confirmation."
    )
    ...
    if freeze["status"] != "confirmation_authorized_and_frozen":
        raise ValueError("V52 confirmation is not authorized")
    candidate = freeze["candidate"]
    if candidate not in CANDIDATES_V52:
        raise ValueError("V52 confirmation candidate is outside the registry")
    checks = {
        ...
        "dense margin manifest": (
            sha256_file(args.margin_manifest),
            freeze["dense_margin_manifest_sha256"],
        ),
        ...
        "model artifact": (
            sha256_file(args.model_artifact),
            freeze["model_artifact_sha256"],
        ),
        "confirmation data": (
            sha256_file(args.data),
            freeze["confirmation_structural_sha256"],
        ),
    }
    for label, (observed, expected) in checks.items():
        if observed != expected:
            raise ValueError(f"V52 {label} changed after freeze")
    manifest = json.loads(
    ...
    if (
        ...
        raise ValueError("V52 confirmation dense margin data is not admitted")
    with Path(args.model_artifact).open("rb") as handle:
        bundle = pickle.load(handle)
    if bundle["candidate"] != candidate:
        raise ValueError("V52 frozen model candidate differs from the freeze")

    portfolio = protocol["portfolio"]
    panel, matrix, coverage, return_manifest = build_model_source_panel_v52(
    ...
    predictions = predict_frozen_model_v52(bundle, matrix)
    evaluated, exposures = neutralize_model_predictions_v52(
        panel,
        predictions,
        candidate=candidate,
        minimum_rows=int(protocol["final_score_neutralization"]["minimum_rows"]),
    )
    ...
    required_years = set(
        int(value) for value in gate["required_core_years_positive"]
    )
