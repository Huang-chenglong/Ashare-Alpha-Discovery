...
from ashare_alpha.robust_v46 import (
    CANDIDATES_V46,
...
def neutralize_family_v46(
    ...
    *,
    ...
    for candidate in CANDIDATES_V46:
    ...
    if not output:
        raise ValueError("V46 produced no final neutralized month")
    return pd.concat(output, ignore_index=True)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Adaptive E+F+G construction of the frozen V46 family."
    )
    ...
    if registered != CANDIDATES_V46:
        raise ValueError("V46 candidate registry mismatch")
    cache_path = Path(args.cache)
    assert_hash(cache_path, protocol["cache"]["sha256"], "V46 adaptive cache")
    metadata = json.loads(Path(args.cache_metadata).read_text(encoding="utf-8"))
    if not metadata["contains_bucket_g"] or metadata["contains_bucket_h"]:
        raise ValueError("V46 cache evidence roles are invalid")
    if metadata["cache_sha256"] != protocol["cache"]["sha256"]:
        raise ValueError("V46 cache metadata differs from protocol")
    controls = metadata["final_main_effect_controls"]
    panel = build_robust_candidates_v46(pd.read_parquet(cache_path))
    scored = neutralize_family_v46(
        panel,
        controls,
    ...
    summary = apply_family_gate_v46(
        pd.concat(summaries, ignore_index=True),
        q_maximum=float(gate["family_bh_q_max"]),
    )
    selected_candidate = select_candidate_v46(summary)
    ...
    result_metadata = {
        ...
        "candidates": list(CANDIDATES_V46),
    ...
    print(json.dumps(result_metadata, ensure_ascii=False, indent=2))
