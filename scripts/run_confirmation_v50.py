...
from ashare_alpha.margin_factors_v50 import CANDIDATES_V50
from ashare_alpha.v50_pipeline import build_evaluation_panel_v50


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run one-shot asset-and-time-disjoint V50 confirmation."
    )
    ...
    if freeze["status"] != "confirmation_authorized_and_frozen":
        raise ValueError("V50 confirmation is not authorized")
    candidate = freeze["candidate"]
    if candidate not in CANDIDATES_V50:
        raise ValueError("V50 confirmation candidate is outside the registry")
    checks = {
    ...
    for label, (observed, expected) in checks.items():
        if observed != expected:
            raise ValueError(f"V50 {label} changed after freeze")
    ...
    if (
        ...
        raise ValueError("V50 confirmation dense margin data is not admitted")
