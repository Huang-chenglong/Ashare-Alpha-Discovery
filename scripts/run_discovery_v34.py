...
from ashare_alpha.factors_v34 import (
    CANDIDATES_V34,
    CANDIDATE_COLUMNS_V34,
    CONTROL_COLUMNS_BY_CANDIDATE_V34,
    EXCLUDED_INDUSTRIES_V34,
    FINANCIAL_MAIN_EFFECTS_V34,
    MAXIMUM_ANNOUNCEMENT_LAG_DAYS_V34,
    MAXIMUM_SIGNAL_AGE_DAYS_V34,
    build_monthly_financial_panel_v34,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the frozen V34 cash-lead interaction.")
    ...
    if protocol["candidate_registry"] != CANDIDATE_COLUMNS_V34:
        raise ValueError("V34 protocol candidate registry mismatch")
    expected_prior = int(protocol["multiplicity"]["prior_tests_included"])
    if int(protocol["multiplicity"]["total_tests_after_v34"]) != expected_prior + 1:
        raise ValueError("V34 protocol multiplicity total is inconsistent")
    filters = protocol["financial_data"]["filters"]
    if int(filters["announcement_lag_days"][1]) != MAXIMUM_ANNOUNCEMENT_LAG_DAYS_V34:
        raise ValueError("V34 announcement-lag constant differs from protocol")
    if int(filters["maximum_signal_age_days"]) != MAXIMUM_SIGNAL_AGE_DAYS_V34:
        raise ValueError("V34 staleness constant differs from protocol")
    if tuple(filters["excluded_shenwan_l1_codes"]) != EXCLUDED_INDUSTRIES_V34:
        raise ValueError("V34 industry exclusions differ from protocol")
    if protocol["neutralization"]["financial_main_effects"] != FINANCIAL_MAIN_EFFECTS_V34:
        raise ValueError("V34 financial controls differ from protocol")
    ...
    if len(prior) != expected_prior or prior["candidate"].nunique() != expected_prior:
        raise ValueError(f"V34 requires exactly {expected_prior} unique prior tests")

    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=True)
    financial, financial_audit, financial_manifest_path = load_validated_financial_history(
        Path(args.financial_directory)
    )
    quarterly = build_quarterly_financial_features_v34(financial)
    daily, prices = load_formation_daily(args.data, args.hfq_cache)
    raw = build_monthly_financial_panel_v34(daily, financial)
    features = finalize_candidates_v34(
        align_industry_history(raw, pd.read_parquet(args.industry))
    )
    complete = features[["clpi_v34", *FINANCIAL_MAIN_EFFECTS_V34]].notna().all(axis=1)
    coverage = (
        features.assign(
            complete_financial=complete,
            nonzero_interaction=features["clpi_v34"].gt(0.0),
        )
        .groupby("date")
        .agg(
            universe=("asset", "size"),
    ...
    candidate = CANDIDATE_COLUMNS_V34[0]
    controls = CONTROL_COLUMNS_BY_CANDIDATE_V34[candidate]
    ...
    summary, yearly, monthly = evaluate_research(
        evaluated,
        candidate_columns=[candidate],
        candidate_definitions=CANDIDATES_V34,
    ...
    selection = {
        ...
        "financial_vintage_limitation": protocol["financial_data"]["vintage_limitation"],
