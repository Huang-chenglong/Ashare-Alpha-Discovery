...
from ashare_alpha.factors_v8 import (
    CANDIDATES_V8,
    CANDIDATE_COLUMNS_V8,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen PDS candidates on bucket C only.")
    parser.add_argument("--data", required=True)
    parser.add_argument("--hfq-cache", required=True)
    parser.add_argument("--industry", required=True)
    parser.add_argument("--prior-summary", action="append", required=True)
    parser.add_argument("--protocol", default="configs/discovery_v8.yaml")
    parser.add_argument("--output", default="reports/discovery_v8")
    args = parser.parse_args()

    prior = pd.concat([pd.read_csv(path) for path in args.prior_summary], ignore_index=True)
    if len(prior) != 49 or prior["candidate"].nunique() != 49:
        raise ValueError("V8 requires exactly all 49 unique prior candidate tests")
    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=True)
    daily, prices = load_formation_daily(args.data, args.hfq_cache)
    features = compute_candidates_v8(daily)
    ...
    panels = pd.concat(
        [
            neutralized_candidate_panels(
                ...
                control_columns=CONTROL_COLUMNS_BY_CANDIDATE_V8[candidate],
                minimum_rows=60,
            )
            for candidate in CANDIDATE_COLUMNS_V8
        ],
        ignore_index=True,
    )
    ...
    summary, yearly, monthly = evaluate_research(
        evaluated,
        candidate_columns=CANDIDATE_COLUMNS_V8,
        candidate_definitions=CANDIDATES_V8,
    ...
    metadata = {
        "experiment_id": "participation-distribution-dominance-v8",
        ...
        "current_test_count": len(CANDIDATE_COLUMNS_V8),
        "multiplicity_family_size": len(prior) + len(CANDIDATE_COLUMNS_V8),
