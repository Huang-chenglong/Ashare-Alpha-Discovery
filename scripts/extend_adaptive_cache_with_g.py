...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extend the consumed E+F component cache with consumed G."
    )
    parser.add_argument("--base-cache", required=True)
    parser.add_argument("--base-metadata", required=True)
    parser.add_argument("--g-data", required=True)
    parser.add_argument("--g-hfq-cache", required=True)
    ...
    parser.add_argument("--g-protocol", required=True)
    ...
    assert_hash(base_path, base_metadata["cache_sha256"], "E+F base cache")
    if base_metadata["contains_bucket_g"]:
        raise ValueError("Base cache already contains bucket G")
    protocol = yaml.safe_load(
        Path(args.g_protocol).read_text(encoding="utf-8")
    )
    assert_hash(
        Path(args.g_data),
    ...
    if final_controls != base_metadata["final_main_effect_controls"]:
        raise ValueError("G controls differ from the E+F cache")
    ...
    g_cache = panel[output_columns].merge(
        components,
        on=["date", "asset"],
        how="inner",
        validate="one_to_one",
    ...
    g_cache = g_cache[
        g_cache["date"].between("2023-01-01", "2025-12-31")
    ].copy()

    base = pd.read_parquet(base_path)
    if list(base.columns) != list(g_cache.columns):
        raise ValueError("E+F and G cache schemas differ")
    base_assets = set(base["asset"].astype(str).str.zfill(6))
    g_assets = set(g_cache["asset"].astype(str).str.zfill(6))
    if base_assets.intersection(g_assets):
        raise ValueError("Consumed G overlaps the E+F cache")
    combined = (
        pd.concat([base, g_cache], ignore_index=True)
        .sort_values(["date", "asset"])
        .reset_index(drop=True)
    )
    if combined.duplicated(["date", "asset"]).any():
        raise ValueError("E+F+G cache has duplicate date/asset rows")
    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    combined.to_parquet(output_path, index=False)
    metadata = {
        "role": "consumed_E_plus_F_plus_G_adaptive_component_cache",
        "contains_bucket_g": True,
        "contains_bucket_h": False,
        "rows": len(combined),
        "assets": int(combined["asset"].nunique()),
        "months": int(combined["date"].nunique()),
        "minimum_date": str(combined["date"].min().date()),
        "maximum_date": str(combined["date"].max().date()),
        "columns": list(combined.columns),
        "final_main_effect_controls": final_controls,
        "base_cache_sha256": sha256(base_path),
        "g_data_sha256": sha256(Path(args.g_data)),
        ...
        "return_manifests_G": return_manifests,
    }
