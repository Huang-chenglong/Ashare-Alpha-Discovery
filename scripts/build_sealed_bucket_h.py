...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build sealed asset-disjoint bucket H without factor outcomes."
    )
    parser.add_argument("--candidate-data", required=True)
    parser.add_argument("--exclude-data", action="append", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--minimum-assets", type=int, default=160)
    parser.add_argument("--minimum-monthly-members", type=int, default=160)
    ...
    manifest.update(
        {
            "role": "sealed formula-unseen bucket H for V46 and later",
            "selection": (
                "All historical CSI300 source assets whose code never appears "
                "in consumed construction/confirmation buckets E, F, or G"
            ),
