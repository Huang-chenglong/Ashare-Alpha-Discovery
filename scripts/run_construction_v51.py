...
from ashare_alpha.margin_factors_v51 import (
    CANDIDATES_V51,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run frozen adaptive V51 bucket-A construction."
    )
    ...
    if tuple(protocol["candidate_registry"]) != CANDIDATES_V51:
        raise ValueError("V51 registry differs from the frozen implementation")
    if protocol["status"] != "frozen_after_v50_A_failure_before_v51_A_outcomes":
        raise ValueError("V51 protocol status is invalid")
    ...
    checks = {
        "dense margin": (
            ...
            frozen["dense_margin_data_sha256"],
        ),
        "margin manifest": (
            sha256_file(manifest_path),
            frozen["dense_margin_manifest_sha256"],
        ),
        "structural": (
            sha256_file(args.data),
            frozen["construction_structural_sha256"],
        ),
        "industry": (sha256_file(args.industry), frozen["industry_sha256"]),
    }
    for label, (observed, expected) in checks.items():
        if observed != expected:
            raise ValueError(f"V51 {label} hash differs from the freeze")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if (
        manifest["audit"]["status"] != "admitted"
        or manifest["output_sha256"] != sha256_file(margin_path)
    ):
        raise ValueError("V51 dense margin data is not admitted")
    ...
    summary = apply_construction_gate_v51(
    ...
    selected_candidate = select_candidate_v51(summary)
    ...
    construction = {
        ...
        "candidates": list(CANDIDATES_V51),
    ...
    freeze = {
        "experiment_id": "persistent-weekly-margin-path-v51-confirmation",
        "status": (
