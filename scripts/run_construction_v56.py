...
from ashare_alpha.model_v56 import (
    CANDIDATE_V56,
    fit_final_model_v56,
    walk_forward_predictions_v56,
)
from ashare_alpha.v56_pipeline import (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen V56 A construction.")
    for name in [
        "data", "hfq-cache", "industry", "dense-margin", "margin-manifest",
        "unseen-manifest", "protocol", "output",
    ]:
        parser.add_argument(f"--{name}", required=True)
    args = parser.parse_args()
    protocol_path = Path(args.protocol)
    protocol = yaml.safe_load(protocol_path.read_text(encoding="utf-8"))
    if protocol["candidate"] != CANDIDATE_V56:
        raise ValueError("V56 candidate differs from implementation")
    if protocol["status"] != "frozen_after_v55_A_failure_before_v56_outcomes":
        raise ValueError("V56 protocol status is invalid")
    ...
    for path, expected in checks.items():
        if sha256_file(path) != expected:
            raise ValueError(f"V56 frozen input changed: {path}")
    margin_manifest = json.loads(
        Path(args.margin_manifest).read_text(encoding="utf-8")
    )
    if margin_manifest["audit"]["status"] != "admitted":
        raise ValueError("V56 margin data is not admitted")

    portfolio = protocol["portfolio"]
    panel, matrix, coverage, return_manifest = build_model_source_panel_v56(
    ...
    prediction = walk_forward_predictions_v56(
        panel,
        matrix,
    ...
    evaluated, exposures = neutralize_model_predictions_v56(
        panel,
        prediction,
    ...
    selected = CANDIDATE_V56 if bool(summary.iloc[0]["passes_construction_gate"]) else None
