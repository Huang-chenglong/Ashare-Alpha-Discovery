...
from ashare_alpha.factors_v39 import (
    CANDIDATES_V39,
    CANDIDATE_COLUMNS_V39,
    CONTROL_COLUMNS_BY_CANDIDATE_V39,
    FINANCIAL_MAIN_EFFECTS_V39,
    MAXIMUM_SIGNAL_AGE_DAYS_V39,
    build_monthly_financial_panel_v39,
    build_quarterly_financial_features_v39,
    finalize_candidates_v39,
)
from ashare_alpha.statistics import hac_mean_test
from ashare_alpha.tdx_financial import FIELDS_V39
from run_discovery_v33 import load_validated_financial_history, sha256


def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen V39 customer-funding working-capital alignment.")
    ...
    if protocol["candidate_registry"] != CANDIDATE_COLUMNS_V39:
        raise ValueError("V39 protocol candidate registry mismatch")
    expected_prior = int(protocol["multiplicity"]["prior_tests_included"])
    if int(protocol["multiplicity"]["total_tests_after_v39"]) != expected_prior + 1:
        raise ValueError("V39 multiplicity total is inconsistent")
    if int(protocol["financial_data"]["maximum_signal_age_days"]) != MAXIMUM_SIGNAL_AGE_DAYS_V39:
        raise ValueError("V39 signal age differs from protocol")
    if protocol["neutralization"]["financial_main_effects"] != FINANCIAL_MAIN_EFFECTS_V39:
        raise ValueError("V39 financial controls differ from protocol")
    ...
    if len(prior) != expected_prior or prior["candidate"].nunique() != expected_prior:
        raise ValueError(f"V39 requires exactly {expected_prior} unique prior tests")

    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=True)
    financial, financial_audit, manifest_path = load_validated_financial_history(
        Path(args.financial_directory), field_map=FIELDS_V39
    )
    quarterly = build_quarterly_financial_features_v39(financial)
    daily, prices = load_formation_daily(args.data, args.hfq_cache)
    raw = build_monthly_financial_panel_v39(daily, financial)
    features = finalize_candidates_v39(
        align_industry_history(raw, pd.read_parquet(args.industry))
    )
    complete = features[["cfwa_v39", *FINANCIAL_MAIN_EFFECTS_V39]].notna().all(axis=1)
    coverage = (
        features.assign(
            complete_financial=complete,
            nonzero_interaction=features["cfwa_v39"].gt(0.0),
        )
        .groupby("date")
        .agg(
            universe=("asset", "size"),
    ...
    candidate = CANDIDATE_COLUMNS_V39[0]
    controls = CONTROL_COLUMNS_BY_CANDIDATE_V39[candidate]
    ...
    summary, yearly, monthly = evaluate_research(
        evaluated,
        candidate_columns=[candidate],
        candidate_definitions=CANDIDATES_V39,
