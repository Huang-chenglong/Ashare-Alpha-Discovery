...
from ashare_alpha.model_v53 import CANDIDATES_V53, predict_frozen_model_v53
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run one-shot asset-and-time-disjoint V53 confirmation."
    )
    ...
    if freeze["status"] != "confirmation_authorized_and_frozen":
        raise ValueError("V53 confirmation is not authorized")
    candidate = freeze["candidate"]
    if candidate not in CANDIDATES_V53:
        raise ValueError("V53 confirmation candidate is outside the registry")
    checks = {
    ...
    for label, (observed, expected) in checks.items():
        if observed != expected:
            raise ValueError(f"V53 {label} changed after freeze")
    manifest = json.loads(
    ...
    if (
        ...
        raise ValueError("V53 confirmation dense margin data is not admitted")
    with Path(args.model_artifact).open("rb") as handle:
        bundle = pickle.load(handle)
    if bundle["candidate"] != candidate:
        raise ValueError("V53 frozen model candidate differs from the freeze")
    ...
    predictions = predict_frozen_model_v53(bundle, matrix)
