...
from ashare_alpha.margin_factors_v49 import CANDIDATES_V49
from ashare_alpha.v49_pipeline import build_evaluation_panel_v49


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run one-shot asset-and-time-disjoint V49 confirmation."
    )
    ...
    if freeze["status"] != "confirmation_authorized_and_frozen":
        raise ValueError("V49 confirmation is not authorized")
    candidate = freeze["candidate"]
    if candidate not in CANDIDATES_V49:
        raise ValueError("V49 confirmation candidate is outside the registry")
    required_hashes = {
        "protocol": (sha256_file(protocol_path), freeze["protocol_sha256"]),
        "margin": (sha256_file(args.margin), freeze["margin_data_sha256"]),
    ...
    for label, (observed, expected) in required_hashes.items():
        if observed != expected:
            raise ValueError(f"V49 {label} changed after confirmation freeze")
    ...
    if (
        ...
        raise ValueError("V49 confirmation margin data is not admitted")
