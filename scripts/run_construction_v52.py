...
def _require_hash(path: str | Path, expected: str, label: str) -> None:
    if sha256_file(path) != expected:
        raise ValueError(f"V52 {label} differs from the frozen protocol")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run frozen V52 nonlinear margin-model construction."
    )
    ...
    if tuple(protocol["candidate_registry"]) != CANDIDATES_V52:
        raise ValueError("V52 registry differs from the frozen implementation")
    if (
        protocol["status"]
        != "frozen_after_v51_A_failure_before_v52_A_model_outcomes"
    ):
        raise ValueError("V52 protocol status is invalid")
    ...
    if (
        ...
        raise ValueError("V52 dense margin data is not admitted")
    ...
    summary = apply_construction_gate_v52(
    ...
    summary["passes_data_coverage_gate"] = (
        summary["candidate"].map(coverage_gate_by_candidate).fillna(False)
    )
    summary["passes_construction_gate"] &= summary[
        "passes_data_coverage_gate"
    ]
    selected_candidate = select_candidate_v52(summary)
    ...
    construction = {
        ...
        "role": "adaptive_A_walk_forward_model_construction",
        "candidates": list(CANDIDATES_V52),
    ...
    freeze = {
        "experiment_id": "nonlinear-margin-path-model-v52-confirmation",
        "status": (
