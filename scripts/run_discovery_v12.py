...
from ashare_alpha.factors_v12 import (
    CANDIDATES_V12,
    CANDIDATE_COLUMNS_V12,
    CONTROL_COLUMNS_BY_CANDIDATE_V12,
    compute_candidates_v12,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen DRCA candidates on bucket C only.")
    parser.add_argument("--data", required=True)
    parser.add_argument("--hfq-cache", required=True)
    parser.add_argument("--industry", required=True)
    parser.add_argument("--prior-summary", action="append", required=True)
    parser.add_argument("--protocol", default="configs/discovery_v12.yaml")
    parser.add_argument("--output", default="reports/discovery_v12")
    args = parser.parse_args()
    prior = pd.concat([pd.read_csv(path) for path in args.prior_summary], ignore_index=True)
    if len(prior) != 57 or prior["candidate"].nunique() != 57:
        raise ValueError("V12 requires exactly all 57 unique prior candidate tests")
    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=True)
    daily, prices = load_formation_daily(args.data, args.hfq_cache)
    features = compute_candidates_v12(daily)
    ...
    panels = pd.concat(
        [
            neutralized_candidate_panels(
                ...
                control_columns=CONTROL_COLUMNS_BY_CANDIDATE_V12[candidate],
                minimum_rows=60,
            )
            for candidate in CANDIDATE_COLUMNS_V12
        ],
        ignore_index=True,
    )
    ...
    summary, yearly, monthly = evaluate_research(
        evaluated,
        candidate_columns=CANDIDATE_COLUMNS_V12,
        candidate_definitions=CANDIDATES_V12,
    ...
    metadata = {
        "experiment_id": "directional-range-conversion-asymmetry-v12",
        "research_bucket": "C/hash-0",
        "adaptive_reuse_disclosed": True,
        "prior_test_count": len(prior),
        "current_test_count": len(CANDIDATE_COLUMNS_V12),
        "multiplicity_family_size": len(prior) + len(CANDIDATE_COLUMNS_V12),
