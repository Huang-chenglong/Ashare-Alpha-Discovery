...
from ashare_alpha.factors_v14 import (
    CANDIDATES_V14,
    CANDIDATE_COLUMNS_V14,
    CONTROL_COLUMNS_BY_CANDIDATE_V14,
    compute_candidates_v14,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen VSTI candidates on bucket C only.")
    ...
    parser.add_argument("--protocol", default="configs/discovery_v14.yaml")
    parser.add_argument("--output", default="reports/discovery_v14")
    args = parser.parse_args()
    prior = pd.concat([pd.read_csv(path) for path in args.prior_summary], ignore_index=True)
    if len(prior) != 59 or prior["candidate"].nunique() != 59:
        raise ValueError("V14 requires exactly all 59 unique prior candidate tests")
    ...
    if vwap_audit.get("confirmation_bucket_opened") is not False:
        raise ValueError("V14 research manifest must attest that confirmation remains unopened")
    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=True)
    daily, prices = load_formation_daily(args.data, args.hfq_cache)
    features = compute_candidates_v14(daily)
    ...
    panels = pd.concat(
        [
            neutralized_candidate_panels(
                ...
                control_columns=CONTROL_COLUMNS_BY_CANDIDATE_V14[candidate],
                minimum_rows=60,
            )
            for candidate in CANDIDATE_COLUMNS_V14
        ],
        ignore_index=True,
    )
    ...
    summary, yearly, monthly = evaluate_research(
        evaluated,
        candidate_columns=CANDIDATE_COLUMNS_V14,
        candidate_definitions=CANDIDATES_V14,
    ...
    metadata = {
        ...
        "current_test_count": len(CANDIDATE_COLUMNS_V14),
        "multiplicity_family_size": len(prior) + len(CANDIDATE_COLUMNS_V14),
