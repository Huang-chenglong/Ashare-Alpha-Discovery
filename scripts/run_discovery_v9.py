...
from ashare_alpha.factors_v9 import (
    CANDIDATES_V9,
    CANDIDATE_COLUMNS_V9,
    CONTROL_COLUMNS_BY_CANDIDATE_V9,
    compute_candidates_v9,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen DPDA candidates on bucket C only.")
    parser.add_argument("--data", required=True)
    parser.add_argument("--hfq-cache", required=True)
    parser.add_argument("--industry", required=True)
    parser.add_argument("--prior-summary", action="append", required=True)
    parser.add_argument("--protocol", default="configs/discovery_v9.yaml")
    parser.add_argument("--output", default="reports/discovery_v9")
    args = parser.parse_args()
    prior = pd.concat([pd.read_csv(path) for path in args.prior_summary], ignore_index=True)
    if len(prior) != 52 or prior["candidate"].nunique() != 52:
        raise ValueError("V9 requires exactly all 52 unique prior candidate tests")

    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=True)
    daily, prices = load_formation_daily(args.data, args.hfq_cache)
    features = compute_candidates_v9(daily)
    aligned = align_industry_history(
        month_end_formation(features), pd.read_parquet(args.industry)
    )
    panels = pd.concat(
        [
            neutralized_candidate_panels(
                ...
                control_columns=CONTROL_COLUMNS_BY_CANDIDATE_V9[candidate],
                minimum_rows=60,
            )
            for candidate in CANDIDATE_COLUMNS_V9
        ],
        ignore_index=True,
    )
    ...
    summary, yearly, monthly = evaluate_research(
        evaluated,
        candidate_columns=CANDIDATE_COLUMNS_V9,
        candidate_definitions=CANDIDATES_V9,
    ...
    metadata = {
        "experiment_id": "directional-participation-dispersion-asymmetry-v9",
        "claim_type": "adaptive new primitive candidate",
        "research_bucket": "C/hash-0",
        "adaptive_reuse_disclosed": True,
        "prior_test_count": len(prior),
        "current_test_count": len(CANDIDATE_COLUMNS_V9),
        "multiplicity_family_size": len(prior) + len(CANDIDATE_COLUMNS_V9),
