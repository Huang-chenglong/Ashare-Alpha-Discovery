...
from ashare_alpha.evaluate import evaluate_research, exposure_diagnostics, neutralized_candidate_panels
from ashare_alpha.factors_v35 import normalize_protocol_periods_v35
from ashare_alpha.factors_v40 import (
    CANDIDATES_V40, CANDIDATE_COLUMNS_V40, CONTROL_COLUMNS_BY_CANDIDATE_V40,
    FINANCIAL_MAIN_EFFECTS_V40, MAXIMUM_SIGNAL_AGE_DAYS_V40,
    build_monthly_financial_panel_v40, build_quarterly_financial_features_v40,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen V40 customer-financing momentum alignment.")
    ...
    if protocol["candidate_registry"] != CANDIDATE_COLUMNS_V40:
        raise ValueError("V40 protocol registry mismatch")
    expected_prior = int(protocol["multiplicity"]["prior_tests_included"])
    if int(protocol["multiplicity"]["total_tests_after_v40"]) != expected_prior + 1:
        raise ValueError("V40 multiplicity total is inconsistent")
    if int(protocol["financial_data"]["maximum_signal_age_days"]) != MAXIMUM_SIGNAL_AGE_DAYS_V40:
        raise ValueError("V40 signal age differs from protocol")
    if protocol["neutralization"]["financial_main_effects"] != FINANCIAL_MAIN_EFFECTS_V40:
        raise ValueError("V40 controls differ from protocol")
    ...
    if len(prior) != expected_prior or prior["candidate"].nunique() != expected_prior:
        raise ValueError(f"V40 requires exactly {expected_prior} unique prior tests")
    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=True)
    financial, audit, manifest = load_validated_financial_history(
    ...
    raw = build_monthly_financial_panel_v40(daily, financial)
    features = finalize_candidates_v40(
        align_industry_history(raw, pd.read_parquet(args.industry))
    )
    complete = features[["cfma_v40", *FINANCIAL_MAIN_EFFECTS_V40]].notna().all(axis=1)
    coverage = (
        features.assign(
            complete_financial=complete,
            nonzero_interaction=features["cfma_v40"].gt(0.0),
        )
        .groupby("date")
        .agg(
            universe=("asset", "size"),
    ...
    candidate = CANDIDATE_COLUMNS_V40[0]
    controls = CONTROL_COLUMNS_BY_CANDIDATE_V40[candidate]
    panels = neutralized_candidate_panels(
        features, candidate_columns=[candidate], control_columns=controls,
    ...
    gate, portfolio = protocol["research_gate"], protocol["portfolio"]
    summary, yearly, monthly = evaluate_research(
        evaluated, candidate_columns=[candidate], candidate_definitions=CANDIDATES_V40,
        prior_discovery_p_values=prior["discovery_p_one_sided"],
        discovery_start=periods["discovery"][0], discovery_end=periods["discovery"][1],
    ...
    validation_ic = monthly.loc[
        monthly["date"].between(periods["internal_validation"][0], periods["internal_validation"][1]),
        "rank_ic",
    ]
    ...
    summary["passes_research_gate"] &= summary["validation_hac_p_one_sided"].le(
        float(gate["validation_hac_p_one_sided_max"])
    )
    ...
    selection = {
        ...
        "prior_test_count": len(prior), "current_test_count": 1,
        "multiplicity_family_size": len(prior) + 1,
        "research_data_sha256": sha256(Path(args.data)),
        "industry_sha256": sha256(Path(args.industry)),
        "financial_manifest_sha256": sha256(manifest),
        "protocol_sha256": sha256(protocol_path),
        "financial_package_count": int(audit["package_count"]),
